...
vec4 readSkybox(float occlusion, vec3 direction) {
    return vec4(lightColor[0], occlusion);
}
...
void main() {
    ...
    if (landMask(sceneDepth)) {
        ...
        if (isEyeInWater == 0) {
            sceneColor  = simpleFog(sceneColor, length(scenePos), lightColor[0] * rpi);
        }
    }

    if (isEyeInWater == 1) {
        sceneColor  = waterFog(sceneColor, length(scenePos), lightColor[0]);
    }
