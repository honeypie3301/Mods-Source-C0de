...
shadowData getShadowRegular(vec3 scenePos, float sigma) {
    ...
    float warp      = 1.0;
        ...
        vec4 shadow     = shadowFiltered(pos, 0.0001 / warp);
...
void main() {
    ...
    if (landMask(sceneDepth)) {
        ...
        if (isSSS) shadow   = getShadowSubsurface(diffuseLit, scenePos, shadowSigma, viewDir, stex(colortex0).rgb, sssOpacity);
        else if (diffuseLit) shadow = getShadowRegular(scenePos, shadowSigma);
        ...
        vec3 emission       = mix(sqr(normalize(albedo)), vec3(1.0), emitterLum) * emitterLum * maxOf(lightColor[2]);
            emission        = max(emission, GetBlocklightColor(lightColor[2], lightmaps.x) * 0.71);

        IndirectSources = directSunlight * lightColor[0] * sqrt2;
