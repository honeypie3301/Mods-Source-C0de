...
public class RotOnEntityTickUpdateProcedure {
	...
	private static final double DIE_KICK_SPEED = 6.0;               // Configurable high diagonal/dive speed of the Die Rider Kick
	...
	public static double OMNI_SONIC_BOOM_CD = 300.0;               // Cooldown of the ability in ticks (300 ticks = 15s)
	public static double OMNI_SONIC_BOOM_ANIMATION_TICKS = 190.0;  // Total duration of the animation (9.5s = 190 ticks)
	public static double OMNI_SONIC_BOOM_TRIGGER_TICK = 20.0;      // Remainder of ticks when sonic burst is released
...
private static float lerpAngle(float pct, float start, float end) {
	...
	public static void onServerChat(ServerChatEvent event) {
		...
		if (msg.equals("slam") || msg.equals("overhead") || msg.equals("dropkick") || msg.equals("kick") || msg.equals("die") || msg.equals("rider") || msg.equals("kickrider") || msg.equals("triple") || msg.equals("highsky") || msg.equals("punchkick") || msg.equals("punchrider") || msg.equals("testoverhead") || msg.equals("testrider") || msg.equals("resetanim") || msg.equals("sonic") || msg.equals("sonicboom") || msg.equals("testsonic") || msg.equals("fall") || msg.equals("falling") || msg.equals("testfall")) {
	...
	private static void executeInternal(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
			...
			if (world instanceof ServerLevel level) {
				...
				for (LivingEntity victim : targets) {
					...
					if (Math.abs(dist - radius) <= 1.5) {
						victim.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_seismic_slam"))), entity), 12.0F);
			...
			if (omniSonicTicks == OMNI_SONIC_BOOM_TRIGGER_TICK) {
				if (world instanceof ServerLevel level) {
					...
					for (LivingEntity targetVictim : targets) {
						...
						targetVictim.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(net.minecraft.core.registries.Registries.DAMAGE_TYPE, net.minecraft.resources.ResourceLocation.parse("the_backwoods:rot_sonic_boom"))), entity), damageVal);
				...
				if (skyWarpTicks == 1) {
					...
					if (world instanceof ServerLevel level) {
						...
						if (combatTarget instanceof LivingEntity livTarget) {
							...
							livTarget.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_overhead_slam"))), entity), (float) (punchDmg * 2.2));
						}
					}
				}
			}
				...
				if (world instanceof ServerLevel level) {
					...
					for (LivingEntity near : nearby) {
						if (shouldIgnoreCombatFilter(entity) || shouldIgnoreCombatFilter(near) || (!near.getType().is(TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.parse(K_WOODBOUND))) && !(near instanceof RotEntity))) {
							...
							near.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_overhead_slam"))), entity), nearDmg);
			...
			if (judgmentTicks > 20) {
				...
				if (combatTarget instanceof LivingEntity targetLiv) {
					...
					if (world instanceof ServerLevel level) {
						...
						if (isInfinity) {
							level.explode(entity, targetX, impactY + 0.5, targetZ, 6.0F, Level.ExplosionInteraction.MOB);
						} else if (totemActive) {
						...
						targetLiv.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_judgement_dropkick"))), entity), (float) (punchDmg * dropkickDamageMult));
						...
						for (LivingEntity near : nearby) {
							if (shouldIgnoreCombatFilter(entity) || shouldIgnoreCombatFilter(near) || (!near.getType().is(TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.parse(K_WOODBOUND))) && !(near instanceof RotEntity))) {
								...
								near.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_judgement_dropkick"))), entity), baseNearDmg);
			...
			if (minosTicks == 48 || minosTicks == 36 || minosTicks == 24 || minosTicks == 12) {
				if (combatTarget instanceof LivingEntity targetLiv) {
					...
					if (world instanceof ServerLevel level) {
						...
						if (isInfinity) {
							punchDamage *= 2.5F;
							level.explode(entity, targetLiv.getX(), targetLiv.getY() + 0.5, targetLiv.getZ(), 4.0F, Level.ExplosionInteraction.MOB);
						}

						if (ls != null) {
							targetLiv.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_consecutive_punches"))), ls), punchDamage);
						} else {
							targetLiv.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_consecutive_punches"))), entity), punchDamage);
						}
		...
		if (entity instanceof Mob mob) {
			if (entity.getPersistentData().getDouble("sentinel_sonic_scream_ticks") > 0) {
				mob.getNavigation().stop();
			} else {
		...
		if (!isChannelingAbility(entity) && combatTarget instanceof LivingEntity livTarget && (unlockedSonicBoom && unlockedTP)) {
			...
			if (isFlying) {
				...
				if (flightCd <= 0) {
					...
					livTarget.hasImpulse = true;
					if (world instanceof ServerLevel level) {
						livTarget.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_seismic_slam"))), entity), 15.0F);
		...
		if (globalCd <= 0) {
			...
			if (isHotTarget) {
				...
				if (ENABLE_TELEKINESIS && entity.getPersistentData().getBoolean("unlocked_telekinesis") && entity.getPersistentData().getDouble(K_TK_CD) <= 0 && dist <= 12.0 && Math.random() < 0.04) {
	...
	private static void handleAdaptiveEffects(LevelAccessor world, Entity self, Entity target, int mode, double dist) {
		...
		if (mode == 1) { // Thermodynamic Flame Modes
			...
			if (dist <= 1.5 && self.tickCount % 4 == 0) {
				...
				target.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_inferno_laser"))), self), 4.0F);
			}
		...
		if (entity.tickCount % 6 == 0) {
			...
			if (targetVictim instanceof LivingEntity livVictim && livVictim.isAlive() && facePos.distanceTo(livVictim.position()) <= maxRange) {
				...
				if (targetVictim != null) {
					...
					targetVictim.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_inferno_laser"))), entity), sonicDmg + explosionDmg); // Fire heat penetration
		...
		if (entity.tickCount % 6 == 0) {
			...
			if (targetVictim instanceof LivingEntity livVictim && livVictim.isAlive() && facePos.distanceTo(livVictim.position()) <= maxRange) {
				...
				if (targetVictim != null) {
					...
					targetVictim.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_cryo_laser"))), entity), dmg); // Extreme cryo/freeze dmg
	...
	private static void executeSentinelSonicScream(LevelAccessor world, Entity entity, Entity target, int ticksLeft) {
		...
		if (ticksLeft > 20 && ticksLeft <= 200) {
			...
			List<LivingEntity> targets = level.getEntitiesOfClass(LivingEntity.class, entity.getBoundingBox().inflate(24.0), e -> e != entity && isValidTarget(e, entity));
			for (LivingEntity victim : targets) {
	...
	private static void executeGrappleSiphon(LevelAccessor world, Entity self, Entity target, int ticksLeft) {
		...
		if (distance > 2.0) {
			...
			if (ticksLeft % 4 == 0) {
				target.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_extraction_grapple"))), self), 4.0F); // Direct biological drain
	...
	private static void executeTelekinesis(LevelAccessor world, Entity self, Entity target, int ticksLeft) {
		...
		if (ticksLeft <= 0) {
			if (Math.random() < 0.5) {
				...
				target.hasImpulse = true;
				target.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_telekinesis"))), self), 12.0F);
				...
				target.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_telekinesis"))), self), 8.0F);
	...
	private static void executeSentinelPunch(LevelAccessor world, Entity self, Entity target) {
		...
		if (globalCd <= 0 && target instanceof LivingEntity && Math.random() < (self.getPersistentData().getBoolean("sentinel_totem_active") ? 0.85 : 0.28)) { //combo chance
			...
			if (!availableCombos.isEmpty()) {
				// Threat Assessment: If target has extremely high armor, aggressively favor structure-breaking heavy slams
				...
				if (favorHeavy) {
					...
					if (!heavyCombos.isEmpty() && Math.random() < 0.75) {
						selectCombo = heavyCombos.get((int) (Math.random() * heavyCombos.size()));
					} else {
	...
	private static boolean isValidTarget(Entity target, Entity self, boolean ignoreLineOfSight) {
		...
		if (!bypassFactionFilter) {
			if (target instanceof RotEntity || target instanceof Villager || target instanceof AmbientCreature || target instanceof Animal || target instanceof Slime || target instanceof net.minecraft.world.entity.animal.WaterAnimal) return false;
		}
	...
	private static void fireSuperchargedSonicBoomEffectAndDamage(LevelAccessor world, Entity self, Entity target) {
		...
		for (double step = 1.0; step <= range; step += 1.0) {
			...
			for (Entity targetVictim : swept) {
				...
				targetVictim.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(net.minecraft.core.registries.Registries.DAMAGE_TYPE, net.minecraft.resources.ResourceLocation.parse("the_backwoods:rot_sonic_boom"))), self), 32.0F);
	...
	public static void onIncomingDamage(LivingIncomingDamageEvent event) {
		...
		if (entity instanceof RotEntity) {
			if (entity.getPersistentData().getDouble("sentinel_scanning_ticks") > 0) {
				...
				if (attackerEnt == null) attackerEnt = event.getSource().getDirectEntity();
				if (attackerEnt != null) {
					...
					double dh = Math.sqrt(dx * dx + dz * dz);
					float targetPitch = (float) (-(Mth.atan2(dy, dh) * (180F / Math.PI)));
	...
	private static void handleForwardCarveMining(LevelAccessor world, Entity self, Entity target) {
		...
		float multiplier = 12f; // Splinter was 50f
		float base = 15f;       // Splinter was 50f
		float mineThreshold = maxHardness * multiplier + base;
	...
	private static void syncNBTFlags(Entity entity) {
		...
		if (!entity.onGround() && !entity.isInWater() && !entity.isInLava()) {
			if (entity.getDeltaMovement().y() > 0.4 || (entity.getDeltaMovement().y() < -0.15 && entity.fallDistance > 2.0F)) {
				isAirborne = true;
			}
		...
		boolean isGroundCrushing = slamPhase == 3 || skyWarpTicks > 0 || landingTicks > 0 || judgmentTicks > 20;
	...
	private static void executeCrushLandingBlast(LevelAccessor world, Entity self, @Nullable Entity target, boolean shakeCamera) {
		...
		if (isInfinity) {
			level.explode(self, targetX, targetY + 0.5, targetZ, 6.5F, Level.ExplosionInteraction.MOB);
		}
		if (shakeCamera) {
		...
		for (LivingEntity victim : targets) {
			...
			victim.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:" + dmgType))), self), blastDamage);
	...
	private static double findGroundY(LevelAccessor world, Entity entity) {
		...
		for (int i = 0; i < 64; i++) {
			...
			if (belowPos.getY() < world.getMinBuildHeight()) {
				return world.getMinBuildHeight() + 1.0;
			}
	...
	private static boolean handleCustomCombos(LevelAccessor world, Entity entity, @Nullable Entity combatTarget) {
		...
		if (cc1 <= 0 && cc2 <= 0 && cc3 <= 0 && cc4 <= 0) return false;
		...
		if ((cc1 == 1 || cc2 == 1 || cc3 == 1 || cc4 == 1) && entity.distanceTo(combatTarget) > 5.0) {
			if (world instanceof ServerLevel level) {
		...
		if (cc2 > 0) {
			// ==========================================
			// COMBO 2: High Sky Slam (Uppercut -> Peak Rot Jump -> Overhead Slam -> Crush)
			// ==========================================
			...
			if (cc2 == 1) {
				// Launch player up super high (25-45 blocks high)
				if (entity instanceof LivingEntity ls) ls.swing(InteractionHand.MAIN_HAND, true);
				double launchHeightVel = 2.8 + Math.random() * 1.0; // Perfect vertical launch velocity for 25-45 blocks!
				...
				if (world instanceof ServerLevel level) {
					...
					playHostileSound(level, entity.getX(), entity.getY(), entity.getZ(), "entity.iron_golem.attack", 1.5F, 0.6F);
				}
				...
				if (cc2Air >= 15 && combatTarget.getDeltaMovement().y() <= 0.1) {
					// The Rot also jumps to peak level and teleports right above the player
					entity.teleportTo(combatTarget.getX(), combatTarget.getY() + 1.5, combatTarget.getZ());
					entity.setDeltaMovement(0.0, -0.1, 0.0); // Orient slightly downward

					// Trigger the ACTUAL Overhead Slam move!
					entity.getPersistentData().putDouble("rot_overhead_ticks", OVERHEAD_TOTAL_TICKS);
					entity.getPersistentData().putString("overhead_target_uuid", combatTarget.getUUID().toString());

					// Reset punch animations so overhead shows correctly
				...
				// While rot_overhead_ticks > 0, we run the actual overhead move logic and let it execute
				handleOverheadState(world, entity, combatTarget);

				double overheadTicks = entity.getPersistentData().getDouble("rot_overhead_ticks");
				if (overheadTicks <= 0) {
					// Wait for player to slam into the ground after the overhead slam
					if (combatTarget.onGround() || combatTarget.getY() <= entity.getY() - 15) {
						// Trigger the ACTUAL Crush Slam move starting from Phase 1 (with Leap)
						entity.setDeltaMovement(0.0, 1.9, 0.0);
						entity.hasImpulse = true;
						if (world instanceof ServerLevel level) {
							playHostileSound(level, entity.getX(), entity.getY(), entity.getZ(), "entity.iron_golem.attack", 1.5F, 0.8F);
						...
						// Put on cooldown and exit combo state since the actual crush slam takes over!
		...
		if (cc3 > 0) {
			...
			if (cc3 == 1) {
				...
				applyKnockbackAndSync(combatTarget, knockDir.x * 2.2, 0.35, knockDir.z * 2.2);

				// Deal punch damage
				if (world instanceof ServerLevel level) combatTarget.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_consecutive_punches"))), entity), (float) MELEE_PUNCH_DAMAGE);
				...
				entity.getPersistentData().putDouble("sentinel_cc3_ticks", 15); // slightly shorter gap (0.75s) to make it feel more connected!
		...
		if (cc4 > 0) {
			...
			if (cc4 == 1) {
				...
				entity.getPersistentData().putDouble("sentinel_cc4_ticks", 15); // slightly shorter gap (0.75s) to make it feel more connected!
	...
	private static void executeArmorRipChoke(LevelAccessor world, Entity entity, @Nullable Entity combatTarget, int tickRemaining) {
		...
		if (combatTarget instanceof LivingEntity livTarget) {
			...
			if (tickRemaining % CHOKE_DAMAGE_INTERVAL == 0) {
				if (world instanceof ServerLevel level) {
					try {
						livTarget.hurt(new DamageSource(level.holderOrThrow(net.minecraft.resources.ResourceKey.create(net.minecraft.core.registries.Registries.DAMAGE_TYPE, ResourceLocation.parse("the_backwoods:rot_choke_rip")))), (float) CHOKE_DAMAGE);
					} catch (Exception e) {
						livTarget.hurt(livTarget.damageSources().mobAttack(entity instanceof LivingEntity le ? le : null), (float) CHOKE_DAMAGE);
					}
