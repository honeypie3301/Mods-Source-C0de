...
public class ScandereLignumLogOnTickUpdateProcedure {
	...
	private static void runGrowthTick(LevelAccessor world, double x, double y, double z, BlockState blockstate, RandomSource random) {
		...
		// OPTIMIZATION 1: If this block is deeply buried inside the trunk, it does not grow leaves, branches, or climb.
		// We can return immediately to completely eliminate TPS/tick lag for 50-80 tall trees!
		if (hasLogAbove && hasLogBelow) {
			// If there is wood at least 15 blocks above us, let's skip additional tick logic.
			if (world.getBlockState(currentPos.above(15)).is(blockstate.getBlock())) {
				return;
			}
		}
