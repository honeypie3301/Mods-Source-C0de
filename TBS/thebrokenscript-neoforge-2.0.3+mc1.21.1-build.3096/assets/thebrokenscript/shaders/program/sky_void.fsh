...
vec3 pixelate(vec3 coord, ivec3 pixelate_resolution) {
    ...
    float x = round(uv.x * float(pixelate_resolution.x)) / float(pixelate_resolution.x) + 0.0625;
    float y = round(uv.y * float(pixelate_resolution.y)) / float(pixelate_resolution.y) + 0.0625;
    float z = round(uv.z * float(pixelate_resolution.z)) / float(pixelate_resolution.z) + 0.0625;
...
void main()
{
    ...
    vec3 pixelPosition = CalcEyeFromWindow(raw).xyz + PlayerPosition;
    float n = snoise(vec4(pixelPosition, GameTime));
    vec3 pos = pixelate(pixelPosition + vec3(n), ivec3(16));
    if (distance(pos, PlayerPosition) < 2.0) fragColor = vec4(1.0);
    else fragColor = tex;
//    if (distance(pixelPosition, PlayerPosition) < 2.0) {
//        fragColor.rgb = vec3(1.0 - distance(pixelPosition, PlayerPosition) * 0.02);
//    } else {
//        fragColor = tex;
//    }
}
...
//
//    vec4 tex = texture(DiffuseSampler, texCoord);
//    float sceneDepth = texture(DiffuseDepthSampler, texCoord).x;
//    vec3 pixelPosition = CalcEyeFromWindow(sceneDepth).xyz + (PlayerPosition);
//    vec3 pix = pixelate(pixelPosition, ivec3(16));
