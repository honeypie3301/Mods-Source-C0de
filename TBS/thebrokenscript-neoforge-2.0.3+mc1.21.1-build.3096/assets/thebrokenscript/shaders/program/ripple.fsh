...
void main()
{
    ...
    float n = snoise(vec4((pos * 2.0) + vec3(Time), Time )) ;
    float dist = distance(pos + n * max(RippleDelta - 0.5, 0.0), vec3(0.0));
    float mi = 0.4 * RippleDelta;
    float mx = 0.5 * RippleDelta;
    if (dist > mi && dist < mx) fragColor = mix(vec4(1.0), tex, RippleDelta);
    else fragColor = tex;
}
