...
// 1.21.1 and 1.21.8 neoforge compatible
...
public class FamiliarFarLandsFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public FamiliarFarLandsFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        int farLandsThreshold = 6275412; //distance it generates
            ...
            for (int rz = 0; rz < 4; rz++) {
                for (int bx = 0; bx < 4; bx++) {
                    for (int bz = 0; bz < 4; bz++) {
                        ...
                        int depth = -1; // Top-down column depth tracker
                            ...
                            for (int by = 7; by >= 0; by--) {
                                ...
                                if (density > 0.0) {
                                    if (depth == -1) {
                                        // Surface layer
                                        if (currentY >= 62) {
                                            level.setBlock(targetPos, Blocks.GRASS_BLOCK.defaultBlockState(), 16);
                                        } else {
                                            level.setBlock(targetPos, Blocks.DIRT.defaultBlockState(), 16);
                                        }
                                        depth = 0;
                                    } else if (depth < 4) {
                                        // Sub-surface layer
                                        ...
                                        // Deep core layer
                                        level.setBlock(targetPos, Blocks.STONE.defaultBlockState(), 16);
                                    }
                                } else {
                                    level.setBlock(targetPos, Blocks.AIR.defaultBlockState(), 16);
                                    depth = -1; // Reset when striking air pocket or cave
                                }
                            }
                        }
                    }
