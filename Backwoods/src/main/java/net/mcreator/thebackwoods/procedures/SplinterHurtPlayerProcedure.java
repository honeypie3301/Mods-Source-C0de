...
@EventBusSubscriber
public class SplinterHurtPlayerProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, Entity entity, Entity sourceentity) {
		...
		if (!((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, ResourceLocation.parse("the_backwoods:the_grain")))) {
			if (sourceentity instanceof SplinterEntity && entity instanceof Player && Math.random() < 0.1 && sourceentity.getPersistentData().getBoolean("canTeleport") == true) {
